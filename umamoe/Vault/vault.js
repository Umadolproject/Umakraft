...
/**
 * Receive and store an Inspector-approved result.
 *
 * @param {InspectorAcceptance} inspectorResult
 * @returns {Promise<VaultStoreResult>}
 */
export async function receive(inspectorResult) {
  ...
  const result = await storeData(trustedEnvelope);
  if (result.success) {
 ...
 *
 * @param {{ id?: string }} query — omit id to retrieve all records
 * @returns {Promise<VaultRetrieveResult>}
 */
...
export async function update(id, patch) {
  ...
  const result = await updateData(id, patch);
  if (!result.success) {
 ...
 * Only call when explicitly authorized.
 *
 * @param {string} id
 * @returns {Promise<VaultStoreResult>}
 */

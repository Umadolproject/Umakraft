...
The **commands/** directory contains the slash command definitions for all 28 UmaKraft bot commands.
...
|---------|------|--------|-------------|
| `/fan_gain` | [fan_gain.md](fan_gain.md) | Image | Daily, weekly, and monthly fan gain + current daily ranking |
| `/profile` | [profile.md](profile.md) | Image | Full profile dashboard — gains, personal records, milestone badges, monthly history |
| `/leaderboard` | [leaderboard.md](leaderboard.md) | Image | Circle fan-gain leaderboard — daily / weekly / monthly with rank movement |
...
| `/club_gain` | [club_gain.md](club_gain.md) | Image | 30-day club gain history — daily progress, running totals, summary stats |
...
| `/memberlist` | [memberlist.md](memberlist.md) | Embed / Image | Full roster — active and former members |
| `/search_trainer` | [search_trainer.md](search_trainer.md) | Ephemeral image | Search the trainer card database by name, rank, or skill count |
| `/store` | [store.md](store.md) | Ephemeral embed | Save your trainer card to the bot database |
| `/keep` | [keep.md](keep.md) | Ephemeral embed | Mark a trainer card as permanently kept — prevents auto-expiry |
...
| `/help` | [help.md](help.md) | Image | List all bot commands with descriptions |
...
| `/test_milestone` | [test_milestone.md](test_milestone.md) | Manage Guild | Ephemeral image | Preview a milestone announcement without posting |
...
| Type | Visibility | Discord Method |
|------|-----------|---------------|
| **Image** | Public (channel) | `editReply` with `AttachmentBuilder` |
| **Ephemeral image** | Caller only | `editReply` with `AttachmentBuilder`, `ephemeral: true` |
| **Ephemeral embed** | Caller only | `editReply` with `EmbedBuilder`, `ephemeral: true` |
| **Embed / Image** | Public | `editReply` with embed or attachment depending on data |
| **Public + Ephemeral** | Both | `editReply` (public) + `followUp` (ephemeral confirmation) |

---

## Command File Template

Each command definition file follows this standard structure:
...
| Document | Relevance |
|----------|-----------|
| `Distribution/Commands/Commands.md` | Implementation handlers for all commands |
| `Distribution/Discord/Discord.md` | Platform adapter — registers these commands via deploy script |
| `Distribution/Coordinator/Coordinator.md` | Orchestration actions mapped to each command |

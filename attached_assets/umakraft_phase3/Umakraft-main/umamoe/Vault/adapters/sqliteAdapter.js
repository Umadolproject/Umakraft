...
import { getDatabase, queryOne, queryAll, withRead, withWrite } from '../../../core/sqlite.js';
...
export async function updateData(id, patch) {
  ...
  try {
    return withWrite(dbPath, async (db) => {
      const existing = queryOne(db, `SELECT data_json, metadata_json, stored_at, updated_at FROM vault_records WHERE id = ?`, [id]);
      if (!existing) {

...
export async function processTrainer(trainerId, options = {}) {
  ...
  if (!vaultRecord.success || !vaultRecord.data) {
    ...
    return failureEnvelope('Vault.retrieve', vaultRecord.error ?? 'VAULT_RETRIEVE_FAILED', 'Could not retrieve stored record', { trainerId });
  }

  let previousRecord = options.previousVaultRecord ?? null;
  if (!previousRecord) {
    const previousSnapshot = await runStage('Vault.retrievePrevious', retrieve, { id: trainerId, version: 'previous' });
...
export async function processRankings(params = {}) {
  ...
  for (const trainer of trainers) {
    ...
    const previousSnapshot = await runStage('Vault.retrievePrevious', retrieve, { id: trainer.id, version: 'previous' });
    if (!previousSnapshot?.failedAt && previousSnapshot?.success && previousSnapshot?.data) {
      previousRecord = previousSnapshot.data;
    }
  ...
  return successEnvelope('UmamoeRankingsPipeline', { results }, { total: results.length, succeeded });
}
